...
// ../device-protocol/dist/index.js
var PROTOCOL_VERSION = 1;
var StateSchema = external_exports.enum([
  "idle",
  "thinking",
  "executing",
  "waiting_input",
...
var RiskSchema = external_exports.enum(["low", "medium", "high"]);
var ActionTypeSchema = external_exports.enum([
  "run_command",
  "modify_file",
  "network_access",
  "user_input",
...
var Title = external_exports.string().refine((s) => byteLen(s) <= 30, {
  message: "title exceeds 30 bytes"
});
var Body = external_exports.string().refine((s) => byteLen(s) <= 150, {
  message: "body exceeds 150 bytes"
});
var ChoiceLabel = external_exports.string().refine((s) => charLen(s) <= 8, {
...
var WelcomeSchema = Envelope.extend({
  type: external_exports.literal("welcome"),
  server_time_ms: external_exports.number().int(),
  session_id: external_exports.string(),
  heartbeat_interval_ms: external_exports.number().int().positive(),
  max_question_choices: external_exports.number().int().positive().max(4),
  features: external_exports.array(external_exports.enum(["ack", "device_status"])).default([])
});
var StatusSchema = Envelope.extend({
...
var ChoiceSchema = external_exports.object({
  id: external_exports.string().min(1),
  label: ChoiceLabel
});
var QuestionSchema = Envelope.extend({
  ...
  title: Title,
  body: Body.optional(),
  choices: external_exports.array(ChoiceSchema).min(1).max(4),
  expires_at: external_exports.number().int().positive(),
  require_confirm: external_exports.boolean().default(false),
  /** Bridge-side total of inflight questions including this one, ≥ 1. */
  queue_total: external_exports.number().int().min(1).optional()
});
var ErrorMsgSchema = Envelope.extend({
  type: external_exports.literal("error"),
  code: ErrorCodeSchema,
  title: Title,
  body: Body.optional(),
  related_id: external_exports.string().optional()
});
var SessionInfoSchema = Envelope.extend({
  type: external_exports.literal("session_info"),
  session_id: external_exports.string().optional(),
  /** AI-generated thread title (from thread/list.name, falls back to preview). */
  thread_title: external_exports.string().optional(),
  /** "Codex Desktop" / "vscode" / "doudou-bridge" — who created/owns the thread. */
  source: external_exports.string().optional(),
  model: external_exports.string().optional(),
  reasoning_effort: external_exports.string().optional(),
  // "high" / "xhigh" / "auto" ...
  summary_mode: external_exports.string().optional(),
  // "auto" / "none" / "concise" ...
  cwd: external_exports.string().optional(),
  /** Combined "Workspace (on-request)" style label. */
  permissions: external_exports.string().optional(),
  approval_policy: external_exports.string().optional(),
  sandbox: external_exports.string().optional(),
  collaboration_mode: external_exports.string().optional(),
  account_email: external_exports.string().optional(),
  plan_type: external_exports.string().optional(),
  agents_md: external_exports.boolean().optional(),
  git_branch: external_exports.string().optional(),
  cli_version: external_exports.string().optional()
});
...
var ThreadListSchema = Envelope.extend({
  type: external_exports.literal("thread_list"),
  threads: external_exports.array(ThreadSummarySchema).max(20)
});
var UsageLimitSchema = external_exports.object({
  ...
  group_label: external_exports.string().optional(),
  used_pct: external_exports.number().min(0).max(100).optional(),
  window_minutes: external_exports.number().int().positive().optional(),
  resets_at: external_exports.number().int().optional()
});
var UsageSchema = Envelope.extend({
  ...
  session: external_exports.object({
     ...
     */
    current_context_tokens: external_exports.number().int().nonnegative().optional(),
    /** Total tokens the model accepts per turn; used to compute context-window fill %. */
    model_context_window: external_exports.number().int().positive().optional()
  }).optional(),
  limits: external_exports.array(UsageLimitSchema).optional(),
  plan_type: external_exports.string().optional()
});
var HelloSchema = Envelope.extend({
  type: external_exports.literal("hello"),
  device_id: external_exports.string().min(1).max(64),
  fw_version: external_exports.string().max(32),
  pairing_token: external_exports.string().min(8).max(128),
  resume_after_seq: external_exports.number().int().nonnegative().optional()
});
var ReplySchema = Envelope.extend({
  type: external_exports.literal("reply"),
  id: external_exports.string().min(1),
  choice_id: external_exports.string().min(1),
  device_id: external_exports.string().min(1)
});
var FollowThreadSchema = Envelope.extend({
  type: external_exports.literal("follow_thread"),
  thread_id: external_exports.string().min(1)
});
var DeviceStatusSchema = Envelope.extend({
  type: external_exports.literal("device_status"),
  battery_pct: external_exports.number().int().min(0).max(100).optional(),
  charging: external_exports.boolean().optional(),
  rssi: external_exports.number().int().optional(),
  free_heap: external_exports.number().int().nonnegative().optional(),
  uptime_ms: external_exports.number().int().nonnegative().optional()
});
var PingSchema = Envelope.extend({ type: external_exports.literal("ping") });
var PongSchema = Envelope.extend({
  type: external_exports.literal("pong"),
  pong_for_seq: external_exports.number().int().nonnegative()
});
var AckSchema = Envelope.extend({
  type: external_exports.literal("ack"),
  ack_for_seq: external_exports.number().int().nonnegative()
});
var BridgeToDeviceSchema = external_exports.discriminatedUnion("type", [
  WelcomeSchema,
  StatusSchema,
  QuestionSchema,
  ErrorMsgSchema,
...
var DeviceToBridgeSchema = external_exports.discriminatedUnion("type", [
  HelloSchema,
  ReplySchema,
  FollowThreadSchema,
  DeviceStatusSchema,
...
var AnyMessageSchema = external_exports.discriminatedUnion("type", [
  WelcomeSchema,
  StatusSchema,
  QuestionSchema,
  ErrorMsgSchema,
...
// src/ui/marquee.ts
function applyMarquee(host, text) {
  ...
  requestAnimationFrame(() => {
    ...
    span.textContent = `${text}   \xB7   ${text}   \xB7   `;
    host.appendChild(span);
  });
}
function clearMarquee(host) {
...
var Pet = class {
  ...
  restingEye = "normal";
  constructor() {
    this.root = document.getElementById("pet-layer");
    this.lens = this.root.closest(".lens");
    this.statusLine = document.getElementById("status-layer");
    this.titleLine = document.getElementById("title-layer");
    if (this.lens) {
  ...
  setState(state, statusTitle, statusBody) {
    ...
    const text = [statusTitle, statusBody].filter(Boolean).join(" \xB7 ");
  ...
  scheduleBlink() {
    ...
    const delay = 3e3 + Math.random() * 4e3;
...
// src/ui/screens.ts
var ORDER = ["info", "usage", "main", "history"];
var DEFAULT_INDEX = ORDER.indexOf("main");
var ScreenManager = class {
  constructor(opts = {}) {
    this.opts = opts;
    this.track = document.getElementById("track");
    this.lens = this.track.parentElement;
...
function renderQuestion(msg, handlers) {
  const layer = document.getElementById("question-layer");
  const slide = layer.closest(".slide-main");
  ...
  meta.textContent = `${msg.risk.toUpperCase()} \xB7 ${ACTION_LABEL[msg.action_type] ?? msg.action_type}`;
  ...
  const render = () => {
    ...
    if (stage === "confirm") {
      ...
      for (const choice of msg.choices) {
        const variant = msg.risk === "high" && choice.id === "accept" ? "danger" : choice.id === "accept" ? "primary" : "";
...
function renderUsage(usage) {
  const host = document.getElementById("slide-usage-content");
  ...
  if (usage.session) {
    ...
    if (totalBilled !== void 0 && totalBilled > 0) {
  ...
  if (usage.limits?.length) {
    ...
    const orderedKeys = [...grouped.keys()].sort(
      (a, b) => a === void 0 ? -1 : b === void 0 ? 1 : a.localeCompare(b)
    );
    for (const key of orderedKeys) {
      ...
      heading.textContent = key ? `${shortenModelLabel(key)} \xB7 \u5269\u4F59` : usage.plan_type ? `${usage.plan_type.toUpperCase()} \xB7 \u5269\u4F59` : "\u914D\u989D\u5269\u4F59";
...
function shortenModelLabel(s) {
  const parts = s.split(/[-_\s]+/).filter(Boolean);
  const last = parts[parts.length - 1];
  if (last && last.length <= 8 && last.length >= 3) return last;
  return s.length <= 14 ? s : s.slice(0, 13) + "\u2026";
}
function remainingBar(remainPct) {
  const cls = remainPct < 10 ? " crit" : remainPct < 30 ? " warn" : "";
...
function untilHM(epochMs) {
  ...
  const daysAhead = Math.round((startOfTarget - startOfToday) / 864e5);
...
function renderInfo(info) {
  const host = document.getElementById("slide-info-content");
  host.replaceChildren();
  ...
  if (info.account_email || info.plan_type) {
    ...
    const planLine = [info.plan_type?.toUpperCase(), info.collaboration_mode ? `\xB7 ${capitalize(info.collaboration_mode)}` : ""].filter(Boolean).join(" ");
  ...
  if (info.permissions || info.cwd) {
    ...
    if (info.cwd) {
      const el = faintEmpty();
      applyMarquee(el, `\u{1F4C1} ${shortDir(info.cwd)}`);
      block.appendChild(el);
    }
    if (info.agents_md !== void 0) {
...
// src/ui/threadList.ts
var ThreadListView = class {
  host;
  last = [];
  handlers = {};
  constructor(handlers = {}) {
    this.host = document.getElementById("slide-history-content");
  ...
  render(threads) {
    ...
    for (const t of threads.slice(0, 8)) {
      ...
      meta.textContent = [t.source ?? "", t.updated_at ? relTime(t.updated_at) : ""].filter(Boolean).join(" \xB7 ");
...
// src/main.ts
var DEVICE_ID = "doudou-sim";
var FW_VERSION = "0.0.2-sim";
var PAIRING_TOKEN = "dev-token-change-me";
var RECONNECT_DELAY_MS = 1500;
var SimulatorClient = class {
  constructor(url) {
    this.url = url;
    this.pet = new Pet();
  ...
  onRaw(raw) {
    ...
    try {
      ...
      logLine(`\u2717 non-JSON: ${raw.slice(0, 80)}`);
      return;
    }
    const result = parseFromBridge(parsed);
    if (!result.ok) {
      logLine(`\u2717 rejected: ${result.error}`);
      return;
    }
  ...
  dispatch(msg) {
    switch (msg.type) {
      ...
      case "status":
        return this.onStatus(msg);
      case "question":
        return this.onQuestion(msg);
      case "usage":
        return this.onUsage(msg);
      case "session_info":
        return this.onSessionInfo(msg);
      case "thread_list":
        return this.onThreadList(msg);
      case "error":
  ...
  onWelcome(msg) {
    ...
    setConnState("ready");
    setDeviceLabel(`${DEVICE_ID} \xB7 ${msg.session_id}`);
  ...
  onSessionInfo(msg) {
    this.lastInfo = { ...this.lastInfo ?? {}, ...msg };
  ...
  onQuestion(msg) {
    ...
    logLine(`\u2190 question ${msg.id} risk=${msg.risk} expires in ${Math.round(remainingMs / 1e3)}s queue_total=${msg.queue_total ?? "?"}`);
    if (this.current) {
      this.queue.push(msg);
...
var wsUrl = new URL("/device", location.href);
wsUrl.protocol = location.protocol === "https:" ? "wss:" : "ws:";
var client = new SimulatorClient(wsUrl.toString());
client.start();
